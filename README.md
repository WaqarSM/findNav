...
- `Option+Shift+F` opens FindNav by default on macOS. Chrome extension manifests spell this as `Alt+Shift+F`.
- `Alt+Shift+F` opens FindNav by default on Windows/Linux.
- Type to search.
...
If the shortcut does not fire, open `chrome://extensions/shortcuts` and confirm FindNav is assigned to `Option+Shift+F` on macOS or `Alt+Shift+F` on Windows/Linux. Chrome may leave a shortcut unassigned if another browser or system command owns it.
